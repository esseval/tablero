import { initState }             from '../../dungeon/js/state.js';
import { revealAround }          from '../../dungeon/js/fog.js';
import { EVENT_HANDLERS }        from '../../dungeon/js/events.js';
...
import { roll, checkLevelUp, resolveAttackRound, rollSum } from '../../dungeon/js/combat.js';
...
export function toggleEditor() {
  ...
  log(editorMode ? 'Editor activado' : 'Modo juego', 'sys');
...
function itemLabel(item) {
  if (item.type === 'potion') return `Pocion +${item.hp} HP (${item.price} oro)`;
...
function buyItem(item) {
  if (G.player.gold < item.price) { log('No tenes oro suficiente.', 'danger'); return; }
  if (item.stock !== undefined) item.stock--;
  G.player.gold -= item.price;
  if (item.type === 'potion') {
    G.player.hp = Math.min(G.player.hp + item.hp, G.player.maxHp);
    log(`Compraste una pocion. HP: ${G.player.hp}/${G.player.maxHp} | Oro: ${G.player.gold}`, 'ok');
...
export function rollDice() {
  ...
  log(`[dados] ${desc}${n > 1 ? ` = ${total}` : ''} pasos (turno ${G.turns})`, 'sys');
  updateHUD(G);
  boardEl().focus();
}
...
function handleEvent(state, key, event) {
  ...
  if (result.leveledUp) log(`Subiste al nivel ${result.newLevel}! +2 HP maximo`, 'ok');
...
function winGame() {
  ...
  if (leveledUp) log(`Subiste al nivel ${G.player.level}! +2 HP maximo`, 'ok');
  updateHUD(G);
...
export function restartGame(levelList, classId) {
  ...
  updateHUD(G);
  boardEl().focus();
  log(`Bienvenido a "${G.board.meta.name}". Llega a la salida.`, 'sys');
  rollDice();
}
...
function tryOpenDoor(dr, dc) {
  ...
  const failure = roll(1, OPEN_DIFFICULTY) > G.player.dex + rollSum(G.player.dexDice || 1); 
  if (failure) {
    log('La puerta esta atascada. No logras abrirla.', 'danger');
    render(G, boardEl());
    updateHUD(G);
    return;
  }
...
function tryOpenDoorLocked(dr, dc) {
  ...
  if (G.player.keys < 1) {
    log('La puerta esta cerrada con llave. Necesitas una llave.', 'danger');
    return;
  }
  G.player.keys--;
  G.board.map[nr][nc] = 'floor';
...
function moveEnemies() {
  ...
  for (const [startKey, ev] of Object.entries(G.events)) {
    ...
    if (G.events[startKey] !== ev) continue;
...
export function trySearch() {
  ...
  for (const key of G.visible) {
    ...
    G.searched.add(key);
    found.push(event.type === 'potion' ? 'una pocion' : 'una trampa');
  }
